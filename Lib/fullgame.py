...
def game():
    ...
    class Player(pygame.sprite.Sprite):
        ...
        # Shoot bullets button if powerup is active
        def powshoot(self):
            now = pygame.time.get_ticks()
            if self.power >= 2:
    ...
    class Meteors(pygame.sprite.Sprite):
        ...
        def update(self):
            ...
            self.rect.x += self.speedx
            self.rect.y += self.speedy
            if self.rect.top < -60 or self.rect.left < WIDTH - 1150  or self.rect.right > WIDTH + 100 or self.rect.bottom > 560:
                self.kill()
                new_meteors = Meteors()
    ...
    # Player and Enemies and bullets
    ...
    enemie_img = pygame.image.load(path.join(img_dir, "Spaceship.png")).convert()
    ...
    # Powerup images
    powerup_images = {}
    powerup_images['better_gun'] = pygame.image.load(path.join(img_dir , 'bettergun.png')).convert()
    powerup_images['extra_life'] = pygame.image.load(path.join(img_dir , 'extralife.png')).convert()
    ...
    while running:
        ...
        # Make the player invincible when they spawn so they don't die instantly
        ...
        # Get score in order to increase difficulty

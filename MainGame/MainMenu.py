...
#Highscore directory
highscore_dir = path.join(path.dirname(__file__), "Highscore")
...
pygame.mixer.init()
pygame.init()
SpaceMenu_sound = pygame.mixer.music.load(path.join(audio_dir,'SpaceMenu.mp3'))
SpaceGame_sound = pygame.mixer.music.load(path.join(audio_dir,'SpaceGame.mp3'))
Hit_sound = pygame.mixer.music.load(path.join(audio_dir,'hit.mp3'))
Select_sound = pygame.mixer.music.load(path.join(audio_dir,'select.mp3'))
Loselife_sound = pygame.mixer.music.load(path.join(audio_dir,'loselife.mp3'))
Death_sound = pygame.mixer.music.load(path.join(audio_dir,'death.mp3'))
shoot_sound = pygame.mixer.music.load(path.join(audio_dir, 'Shoot.mp3')).convert()
...
BLACK = (0, 0, 0)
BG = pygame.image.load("MainGame/img/space.png")
HS = pygame.image.load("MainGame/img/Startmenu.jpg")
...
def get_font(size): # Returns Press-Start-2P in the desired size
    return pygame.font.Font("MainGame/img/NeoTechItalic-WyKZY.ttf", size)

def play_game():
    pygame.mixer.Channel(1).play(pygame.mixer.Sound(path.join(audio_dir,'SpaceGame.mp3')))
    ...
    while True:
        ...
        class Player(pygame.sprite.Sprite):
            ...
            def shoot(self):
                ...
                pygame.mixer.Channel(2).play(pygame.mixer.Sound(path.join(audio_dir,'Shoot.mp3')))
        ...
        highscore = 0
        saved_highscore = 0
        file = open('MainGame\Highscore\Highscores.txt', 'r')
        for line in file:
        ...
        while running:
            ...
            for hit in hits_bullet_enemie:
                pygame.mixer.music.set_volume(1)
                pygame.mixer.Channel(3).play(pygame.mixer.Sound(path.join(audio_dir,'hit.mp3')))
                score +=1
            ...
            for hit in hits_bullet_meteor:
                pygame.mixer.music.set_volume(1)
                pygame.mixer.Channel(4).play(pygame.mixer.Sound(path.join(audio_dir,'hit.mp3')))
            ...
            for hit in hits_player_enemie:
                pygame.mixer.music.set_volume(1)
                pygame.mixer.Channel(5).play(pygame.mixer.Sound(path.join(audio_dir,'loselife.mp3')))
                player.lives -=1
            ...
            if hits_player_meteors:
                ...
                pygame.mixer.Channel(6).play(pygame.mixer.Sound(path.join(audio_dir,'death.mp3')))
                game_over = True
                previous_score = score
                if score > highscore:
                    highscore = score
                if highscore > saved_highscore:
                    file = open('MainGame\Highscore\Highscores.txt','w')
...
def show_go_screen():
    pygame.mixer.Channel(1).play(pygame.mixer.Sound(path.join(audio_dir,'SpaceMenu.mp3')))
...
def main_menu():
    pygame.mixer.Channel(1).play(pygame.mixer.music.play(path.join(audio_dir,"SpaceMenu.mp3")))
    while True:
        ...
        PLAY_BUTTON = Button(image=pygame.image.load("MainGame/img/Quit Rect.png"), pos=(200, 200), 
                            text_input="PLAY", font=get_font(40), base_color="#fcd4f4", hovering_color="Purple")
        HIGHSCORE_BUTTON = Button(image=pygame.image.load("MainGame/img/highscore Rect.png"), pos=(700, 200), 
                            text_input="HIGHSCORE", font=get_font(40), base_color="#fcd4f4", hovering_color="Purple")
        QUIT_BUTTON = Button(image=pygame.image.load("MainGame/img/Quit Rect.png"), pos=(700, 400), 
                            text_input="QUIT", font=get_font(40), base_color="#fcd4f4", hovering_color="Purple")
        OPTIONS_BUTTON = Button(image=pygame.image.load("MainGame/img/Quit Rect.png"), pos=(200, 400), 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if mouse_presses[0]:
                    if PLAY_BUTTON.checkForInput(MENU_MOUSE_POS):
                        pygame.mixer.music.set_volume(1)
                        pygame.mixer.Channel(7).play(pygame.mixer.Sound(path.join(audio_dir,'Select.mp3')))
                        play_game()
